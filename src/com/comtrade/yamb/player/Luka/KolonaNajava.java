...
public class KolonaNajava {
	
public static PlayerResponse prvoBacanje(Request request) {
	...
	{
		if (request.getBoard().getColumns()[5].getFields().get(13).isPlayable()) 
		{
			PlayerResponse response = new PlayerResponse(false, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.YAMB));
			return response;
		}
	...
	else if(ucestalostKockica[Patterns.vrednostMaxMetode(brojeviNaKockicama)]>3) 
	{
		...
		{
			PlayerResponse response = new PlayerResponse(false, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.POKER));
			return response;
		}
	...
	{
		if (request.getBoard().getColumns()[5].getFields().get(10).isPlayable()) 
		{
			PlayerResponse response = new PlayerResponse(true, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.KENTA));
			return response;
		}
	...
	else if (Patterns.najavaImamoLiFullHouse(brojeviNaKockicama) == true) 
	{
		...
		{
			if (request.getBoard().getColumns()[5].getFields().get(12).isPlayable()) 
			{
				PlayerResponse response = new PlayerResponse(true, cuvanjeKockica,
						new FieldData(Konstante.KOLONA_NAJAVA, Konstante.FULL));
				return response;
			}
		}
		else if (request.getBoard().getColumns()[5].getFields().get(12).isPlayable()) 
		{
			PlayerResponse response = new PlayerResponse(false, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.FULL));
			return response;
	...
	else if (ucestalostKockica[Patterns.vrednostMaxMetode(brojeviNaKockicama)]>=3) 
	{
			...
			{
				if (request.getBoard().getColumns()[5].getFields().get(i).isPlayable() == true) {
					PlayerResponse response = new PlayerResponse(true, cuvanjeKockica,
							new FieldData(Konstante.KOLONA_NAJAVA, i));
					return response;
				}
			}
	...
	//TRILING
		...
		if (request.getBoard().getColumns()[5].getFields().get(11).isPlayable())
		{
			PlayerResponse response = new PlayerResponse(true, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.TRILING));
			return response;
		}
	...
	{
		if (request.getBoard().getColumns()[5].getFields().get(7).isPlayable())
		{
			PlayerResponse response = new PlayerResponse(true, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.MAX));
			return response;
		}
	...
	{
		if (request.getBoard().getColumns()[5].getFields().get(8).isPlayable()) 
		{
			PlayerResponse response = new PlayerResponse(true, cuvanjeKockica,
					new FieldData(Konstante.KOLONA_NAJAVA, Konstante.MIN));
			return response;
		}
